...
public class MainPanel extends BasicPanel {
	...
	btnReports.setToolTipText("GIS Salta - Recaudación - 2009");
	btnGaia.setToolTipText("GIS Salta - Relevamiento Comercial 2009");
	...
	if ((e.getModifiers() & (ActionEvent.SHIFT_MASK)) == ActionEvent.SHIFT_MASK) {
	    MonitorSelector monitorSelector = new MonitorSelector(GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices());
	    monitorSelector.setModal(true);
	    monitorSelector.setVisible(true);
	    parent.setBounds(Environment.graphicsDevice.getDefaultConfiguration().getBounds());
	} else {
