...
import org.jfree.report.modules.output.table.csv.CSVTableModule;
...
public class CSVTableExportDialog extends AbstractExportDialog
{
  ...
  public static final String CSV_OUTPUT_ENCODING
      = "org.jfree.report.modules.gui.csv.Encoding"; //$NON-NLS-1$
  /**
  ...
  private void initialize()
  {
    ...
    if ("true".equals(config.getConfigProperty("org.jfree.report.modules.gui.csv.table.AdvancedSettingsAvailable")))
    {
  ...
  /**
   * Clears all selections, input fields and sets the selected encryption level
   * to none.
   */
  public void clear()
  {
  ...
  protected Configuration grabDialogContents(final boolean full)
  {
    ...
    config.setConfigProperty(CSVTableModule.CONFIGURATION_PREFIX + '.' +
        CSVTableModule.SEPARATOR_KEY, getSeparatorString());
    config.setConfigProperty(CSVTableModule.CONFIGURATION_PREFIX + ".StrictLayout", //$NON-NLS-1$
        String.valueOf(isStrictLayout()));
    config.setConfigProperty(CSVTableExportDialog.CSV_OUTPUT_ENCODING, getEncoding());

    config.setConfigProperty("org.jfree.report.modules.gui.csv.FileName", getFilename()); //$NON-NLS-1$
  ...
  /**
   * Initialises the CSV export dialog from the settings in the report
   * configuration.
   *
  ...
  protected void setDialogContents(final Configuration config)
  {
    ...
    final String tableCSVSeparator = config.getConfigProperty
        (CSVTableModule.CONFIGURATION_PREFIX + '.' + CSVTableModule.SEPARATOR_KEY, CSVTableExportDialog.COMMA_SEPARATOR);
    setSeparatorString(config.getConfigProperty (CSVProcessor.CSV_SEPARATOR, tableCSVSeparator));

    final String strict = config.getConfigProperty(CSVTableModule.CONFIGURATION_PREFIX + ".StrictLayout"); //$NON-NLS-1$
    setStrictLayout("true".equals(strict)); //$NON-NLS-1$

    final String encoding = config.getConfigProperty (CSVTableExportDialog.CSV_OUTPUT_ENCODING, CSVTableExportDialog.CSV_OUTPUT_ENCODING_DEFAULT);
    encodingModel.ensureEncodingAvailable(encoding);
    setEncoding(encoding);

    final String defaultFileName = config.getConfigProperty ("org.jfree.report.modules.gui.csv.FileName"); //$NON-NLS-1$
  ...
  /**
   * Returns the separator string, which is controlled by the selection of radio
   * buttons.
   *
  ...
  protected void performSelectFile()
  {
    if (fileChooser == null)
    {
      ...
      final ExtensionFileFilter filter = new ExtensionFileFilter
          (getResources().getString("csvexportdialog.csv-file-description"), CSVTableExportDialog.CSV_FILE_EXTENSION); //$NON-NLS-1$
  ...
  {
    return "org.jfree.report.modules.gui.csv."; //$NON-NLS-1$
  }
}
