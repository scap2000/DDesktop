...
public class HtmlStreamExportPlugin extends AbstractExportActionPlugin
{
  ...
  {
    resources = new ResourceBundleSupport(HtmlStreamExportPlugin.BASE_RESOURCE_CLASS);
  }
  ...
  {
    return "org.jfree.report.modules.gui.html.export.stream."; //$NON-NLS-1$
  }
  ...
  public boolean performExport (final JFreeReport report)
  {
    final boolean result = performShowExportDialog
        (report, "org.jfree.report.modules.gui.html.stream.Dialog"); //$NON-NLS-1$
    if (result == false)
    {
    ...
    if (isProgressDialogEnabled(report, "org.jfree.report.modules.gui.html.stream.ProgressDialogEnabled"))
    {
    ...
    catch (Exception e)
    {
      ...
      getContext().getStatusListener().setStatus(StatusType.ERROR, getResources().getString("HtmlStreamExportPlugin.USER_FAILED")); //$NON-NLS-1$
      return false;
    }
  }
  ...
  {
    return resources.getMnemonic("action.export-to-html.stream.mnemonic"); //$NON-NLS-1$
  }
