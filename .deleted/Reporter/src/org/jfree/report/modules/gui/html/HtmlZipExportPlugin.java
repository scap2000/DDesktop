...
public class HtmlZipExportPlugin extends AbstractExportActionPlugin
{
  ...
  {
    return "org.jfree.report.modules.gui.html.export.zip."; //$NON-NLS-1$
  }
  ...
  public boolean performExport (final JFreeReport report)
  {
    final boolean result = performShowExportDialog
        (report, "org.jfree.report.modules.gui.html.zip.Dialog"); //$NON-NLS-1$
    if (result == false)
    {
    ...
    if (isProgressDialogEnabled(report, "org.jfree.report.modules.gui.html.zip.ProgressDialogEnabled"))
    {
    ...
    catch (Exception e)
    {
      ...
      getContext().getStatusListener().setStatus(StatusType.ERROR, getResources().getString("HtmlZipExportPlugin.USER_FAILED")); //$NON-NLS-1$
      return false;
    }
  }
  ...
  {
    return resources.getMnemonic("action.export-to-html.zip.mnemonic"); //$NON-NLS-1$
  }
