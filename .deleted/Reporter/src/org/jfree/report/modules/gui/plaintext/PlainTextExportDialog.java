...
public class PlainTextExportDialog extends AbstractExportDialog
{
  ...
  /**
   * Creates a non-modal dialog without a title with the specifed Frame as its
   * owner.
   *
  ...
  /**
   * Creates a non-modal dialog without a title with the specifed Dialog as its
   * owner.
   *
  ...
  private void init()
  {
    ...
    messages = new Messages(PlainTextExportPlugin.BASE_RESOURCE_CLASS);
    ...
    final boolean advancedSettingsTabAvail =
        "true".equals(config.getConfigProperty("org.jfree.report.modules.gui.plaintext.AdvancedSettingsAvailable"));
  ...
  /**
   * clears all selections, input fields and set the selected encryption level
   * to none.
   */
  public void clear()
  {
  ...
  public void setLinesPerInch(final float lpi)
  {
    ...
    throw new IllegalArgumentException(messages.getErrorString("PlainTextExportDialog.ERROR_0001_NO_SUCH_LPI", String.valueOf(lpi))); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  public void setCharsPerInch(final float cpi)
  {
    ...
    if (messages == null)
      messages = new Messages(PlainTextExportPlugin.BASE_RESOURCE_CLASS);
    throw new IllegalArgumentException(messages.getErrorString("PlainTextExportDialog.ERROR_0002_NO_SUCH_CPI", String.valueOf(cpi))); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  protected void setDialogContents(final Configuration config)
  {
    ...
    final String mode = config.getConfigProperty("org.jfree.report.modules.gui.plaintext.ExportType"); //$NON-NLS-1$
    if ("9pin".equals(mode)) //$NON-NLS-1$
    {
      setSelectedPrinter(TYPE_EPSON9_OUTPUT);
    }
    else if ("24pin".equals(mode)) //$NON-NLS-1$
    {
      setSelectedPrinter(TYPE_EPSON24_OUTPUT);
    }
    ...
    final String defaultFileName = config.getConfigProperty
        ("org.jfree.report.modules.gui.plaintext.FileName"); //$NON-NLS-1$
  ...
  protected Configuration grabDialogContents(final boolean full)
  {
    ...
    if (full)
    {
      ...
      {
        case TYPE_EPSON24_OUTPUT:
        {
          config.setConfigProperty
            ("org.jfree.report.modules.gui.plaintext.ExportType", "24pin"); //$NON-NLS-1$ //$NON-NLS-2$
          break;
        }
        case TYPE_EPSON9_OUTPUT:
        {
          config.setConfigProperty
            ("org.jfree.report.modules.gui.plaintext.ExportType", "9pin"); //$NON-NLS-1$ //$NON-NLS-2$
          break;
        }
        case TYPE_IBM_OUTPUT:
        {
          config.setConfigProperty
            ("org.jfree.report.modules.gui.plaintext.ExportType", "ibm"); //$NON-NLS-1$ //$NON-NLS-2$
          break;
        }
        case TYPE_PLAIN_OUTPUT:
        {
          config.setConfigProperty
            ("org.jfree.report.modules.gui.plaintext.ExportType", "plain"); //$NON-NLS-1$ //$NON-NLS-2$
          break;
        }
        default:
      ...
      config.setConfigProperty
          ("org.jfree.report.modules.gui.plaintext.FileName", getFilename()); //$NON-NLS-1$
  ...
  {
    return "org.jfree.report.modules.gui.plaintext."; //$NON-NLS-1$
  }
}
