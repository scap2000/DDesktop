...
public class RTFExportTask implements Runnable
{
       ...
       final SwingGuiContext swingGuiContext) throws ReportProcessingException
  {
    if (report == null)
    {
      throw new NullPointerException(messages.getErrorString("RTFExportTask.RTFExportTask.ERROR_0001_NULL_REPORT")); //$NON-NLS-1$
    }
    ...
    if (filename == null)
    {
      if (messages == null)
      {
        messages = new Messages(RTFExportPlugin.BASE_RESOURCE_CLASS);
      }
      throw new ReportProcessingException(messages.getErrorString("RTFExportTask.RTFExportTask.ERROR_0001_NULL_FILENAME")); //$NON-NLS-1$
    }
  ...
  public void run()
  {
    ...
    try
    {
      ...
      final StreamRTFOutputProcessor target = new StreamRTFOutputProcessor(report.getConfiguration(), out);
      ...
      {
        statusListener.setStatus(StatusType.INFORMATION, messages.getString("RTFExportTask.USER_TASK_FINISHED")); //$NON-NLS-1$
      }
    ...
    catch (ReportInterruptedException re)
    {
      if (statusListener != null)
      {
        statusListener.setStatus(StatusType.WARNING, messages.getString("RTFExportTask.USER_TASK_ABORTED")); //$NON-NLS-1$
      }
      try
      {
        out.close();
    ...
    {
      Log.error("RTF export failed", re); //$NON-NLS-1$
      if (statusListener != null)
      {
        statusListener.setStatus(StatusType.WARNING, messages.getString("RTFExportTask.USER_TASK_FAILED")); //$NON-NLS-1$
      }
