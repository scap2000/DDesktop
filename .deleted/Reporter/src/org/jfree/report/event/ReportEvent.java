 ...
 * <p/>
 * Includes information which {@link org.jfree.report.states.ReportState} generated the event.
 *
...
public class ReportEvent extends EventObject
{
  ...
  /**
   * The event type constant, that this event is a prepare event. Use this constant in combination with one of the other
   ...
   * <p/>
   * The {@link DataRow} is used to access the fields of the {@link org.jfree.report.filter.DataSource} and other
  ...
  /**
   * Checks, whether the deep-traversing flag is set. An event is deep-traversing, if it did not originate in the
   * current report.
   *
  ...
  /**
   * Checks, whether this event is a prepare event. Prepare events are fired before the real event is fired and
   * sometimes simplify the state-management inside complex functions.
   *
   * @return true, if this event is a prepare event, false otherwise.
   */
  public boolean isPrepareEvent()
  {
    return (type & ReportEvent.PREPARE_EVENT) ==
        ReportEvent.PREPARE_EVENT;
  }
}
