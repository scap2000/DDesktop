 ...
 * directly, use the {@link org.jfree.report.function.RowBandingFunction}.
 * <p/>
 ...
 * <p/>
 * The ElementVisibilitySwitchFunction defines two parameters: <ul> <li>element <p>The name of the element(s) in the
 * itemband that should be modified. The element(s) must be named using the "name" attribute.</p> <li>initial-state <p>The initial state of the function. (true or false)
 * defaults to false. This is the reverse of the element's visiblity (set to false to start with an visible element, set
 * to true to hide the element in the first itemrow).</p> </ul>
 *
...
public class ElementVisibilitySwitchFunction extends AbstractFunction implements PageEventListener
{
  ...
  private void triggerVisibleState(final ReportEvent event)
  {
    ...
    else
    {
      if (warned == false)
      {
        Log.warn("The Itemband does not contain an element named " + getElement());
