 ...
 * Time: 18:22:59
 */
public class IndentedLayoutPlugin extends AbstractLayoutPlugin
{
    @NonNls
    @NotNull
    private static final Logger LOG = Logger.getLogger(IndentedLayoutPlugin.class.getName());


    public IndentedLayoutPlugin()
    {
    }
    ...
    {
        return TranslationManager.getInstance().getTranslation("R", "IndentedLayoutPlugin.Name");
    }
    ...
    {
        return IconLoader.getInstance().getTemplateIndentedIcon();
    }
    ...
    public void addVisualReportElements(@NotNull Report report, @NotNull HashMap<String, WizardData> wizardDatas, @NotNull LayoutStyle layoutStyle) throws LayoutException
    {
        ...
        titleLabel.setText(TranslationManager.getInstance().getTranslation("R", "IndentedLayoutPlugin.ReportTitle"));
        ...
        for (int i = 0; i < columnGroups.size(); i++)
        {
            ...
            if (visibleColumnInfoCount > 0)
            {
                ...
                for (int j = 0; j < columnInfo.getColumnInfos().size(); j++)
                {
                    ...
                    if (visibleColumnInfos.contains(info))
                    {
                        ...
                        groupTitleLabel.setPosition(new Point2D.Double(xEnd / visibleColumnInfoCount * visibleIndex + xStart, yStart));
                        groupTitleLabel.setMinimumSize(new DoubleDimension(xEnd / visibleColumnInfoCount - xStart, yEnd - yStart));
            ...
            xStart += pageWidth / visibleColumnInfos.size();
        }


        visibleColumnInfos.removeAll(columnGroups);

        addTableHeader(xStart, tableHeaderPosition, pageWidth, tableHeaderBand, visibleColumnInfos, columnGroups, layoutStyle);

        BandToplevelReportElement itemBand = addItems(xStart, pageWidth, visibleColumnInfos, columnGroups, layoutStyle);
        ...
        {
            report.getHorizontalLinealModel().addGuidLine(new GuideLine(pageWidth / (double) remainingCoumnInfos.size() * i + xStart, true));
        }
    }


    private void addTableHeader(double xStart, double tableHeaderPosition, double pageWidth, @NotNull BandToplevelReportElement tableHeaderBand, @NotNull ArrayList<ColumnInfo> visibleColumnInfos, @NotNull ArrayList<GroupInfo> columnGroups, @NotNull LayoutStyle layoutStyle)
    {
        ArrayList<ColumnInfo> remainingCoumnInfos = new ArrayList<ColumnInfo>(visibleColumnInfos);
        for (GroupInfo groupInfo : columnGroups)
        {
        ...
        double columnWidth = (pageWidth - xStart) / remainingCoumnInfos.size();
        ...
        for (ColumnInfo columnInfo : remainingCoumnInfos)
        {
            ...
            columnTitleLabel.setPosition(new Point2D.Double(xStart, tableHeaderPosition));
    ...
    @NotNull
    private BandToplevelReportElement addItems(double xStart, double pageWidth, @NotNull ArrayList<ColumnInfo> visibleColumnInfos, @NotNull ArrayList<GroupInfo> columnGroups, @NotNull LayoutStyle layoutStyle)
    {
        ArrayList<ColumnInfo> remainingCoumnInfos = new ArrayList<ColumnInfo>(visibleColumnInfos);
        for (GroupInfo groupInfo : columnGroups)
        {
        ...
        for (ColumnInfo columnInfo : remainingCoumnInfos)
        {
            ...
            columnValueTextField.setPosition(new Point2D.Double(xStart, tableHeaderPosition));
