 ...
 * Time: 11:26:24
 */
public class MoveDownOneCommand extends AbstractCommand
{
    public MoveDownOneCommand()
    {
        super("MoveDownOneCommand");
        getTemplatePresentation().setText(TranslationManager.getInstance().getTranslation("R", "MoveDownOneCommand.Text"));
        getTemplatePresentation().setDescription(TranslationManager.getInstance().getTranslation("R", "MoveDownOneCommand.Description"));
        getTemplatePresentation().setMnemonic(TranslationManager.getInstance().getMnemonic("R", "MoveDownOneCommand.Text"));
        getTemplatePresentation().setDisplayedMnemonicIndex(TranslationManager.getInstance().getDisplayedMnemonicIndex("R", "MoveDownOneCommand.Text"));

        getTemplatePresentation().setAccelerator(KeyStroke.getKeyStroke(TranslationManager.getInstance().getTranslation("R", "MoveDownOneCommand.Accelerator")));
    }
    ...
    public void execute(@NotNull CommandEvent event)
    {
        ...
        reportDialog.getUndo().startTransaction(UndoConstants.MOVE_DOWN);
        ReportElement[] reportElements = (ReportElement[]) event.getDataContext().getData(CommandKeys.KEY_SELECTED_ELEMENTS_ARRAY);
        if (reportElements != null)
        {
            for (ReportElement reportElement : reportElements)
            {
                ...
                double y = reportElement.getPosition().y + 1;
