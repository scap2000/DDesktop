 ...
 * Time: 11:26:24
 */
public class DistributeTopCommand extends AbstractPositioningCommand
{
    public DistributeTopCommand()
    {
        super("DistributeTopCommand");
        getTemplatePresentation().setText(TranslationManager.getInstance().getTranslation("R", "DistributeTopCommand.Text"));
        getTemplatePresentation().setDescription(TranslationManager.getInstance().getTranslation("R", "DistributeTopCommand.Description"));
        getTemplatePresentation().setMnemonic(TranslationManager.getInstance().getMnemonic("R", "DistributeTopCommand.Text"));
        getTemplatePresentation().setDisplayedMnemonicIndex(TranslationManager.getInstance().getDisplayedMnemonicIndex("R", "DistributeTopCommand.Text"));

        getTemplatePresentation().setIcon(CommandSettings.SIZE_16, IconLoader.getInstance().getDistributeTopIcon());

        getTemplatePresentation().setAccelerator(KeyStroke.getKeyStroke(TranslationManager.getInstance().getTranslation("R", "DistributeTopCommand.Accelerator")));
    }
    ...
    public void execute(@NotNull CommandEvent event)
    {
        ...
        reportDialog.getUndo().startTransaction(UndoConstants.DISTRIBUTE_TOP);
        ...
        if (reportElements != null && reportElements.length > 2)
        {
            ...
            double height = reportElements[reportElements.length - 1].getPosition().getY() - reportElements[0].getPosition().getY();
            double incr = height / (reportElements.length - 1);
            double currentY = reportElements[0].getPosition().getY();
