 ...
 * Time: 11:26:24
 */
public class DistributeLeftCommand extends AbstractPositioningCommand
{
    public DistributeLeftCommand()
    {
        super("DistributeLeftCommand");
        getTemplatePresentation().setText(TranslationManager.getInstance().getTranslation("R", "DistributeLeftCommand.Text"));
        getTemplatePresentation().setDescription(TranslationManager.getInstance().getTranslation("R", "DistributeLeftCommand.Description"));
        getTemplatePresentation().setMnemonic(TranslationManager.getInstance().getMnemonic("R", "DistributeLeftCommand.Text"));
        getTemplatePresentation().setDisplayedMnemonicIndex(TranslationManager.getInstance().getDisplayedMnemonicIndex("R", "DistributeLeftCommand.Text"));

        getTemplatePresentation().setIcon(CommandSettings.SIZE_16, IconLoader.getInstance().getDistributeLeftIcon());

        getTemplatePresentation().setAccelerator(KeyStroke.getKeyStroke(TranslationManager.getInstance().getTranslation("R", "DistributeLeftCommand.Accelerator")));
    }
    ...
    public void execute(@NotNull CommandEvent event)
    {
        ...
        reportDialog.getUndo().startTransaction(UndoConstants.DISTRIBUTE_LEFT);
        ...
        if (reportElements != null && reportElements.length > 2)
        {
            ...
            double width = reportElements[reportElements.length - 1].getPosition().getX() - reportElements[0].getPosition().getX();
            double incr = width / (reportElements.length - 1);
            double currentX = reportElements[0].getPosition().getX();
