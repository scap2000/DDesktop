 ...
 * Time: 11:26:24
 */
public class LayerDownCommand extends AbstractCommand
{
    public LayerDownCommand()
    {
        super("LayerDownCommand");
        getTemplatePresentation().setText(TranslationManager.getInstance().getTranslation("R", "LayerDownCommand.Text"));
        getTemplatePresentation().setDescription(TranslationManager.getInstance().getTranslation("R", "LayerDownCommand.Description"));
        getTemplatePresentation().setMnemonic(TranslationManager.getInstance().getMnemonic("R", "LayerDownCommand.Text"));
        getTemplatePresentation().setDisplayedMnemonicIndex(TranslationManager.getInstance().getDisplayedMnemonicIndex("R", "LayerDownCommand.Text"));

        getTemplatePresentation().setIcon(CommandSettings.SIZE_16, IconLoader.getInstance().getLayerDownIcon());

        getTemplatePresentation().setAccelerator(KeyStroke.getKeyStroke(TranslationManager.getInstance().getTranslation("R", "LayerDownCommand.Accelerator")));
    }
    ...
    public void execute(@NotNull CommandEvent event)
    {
        ...
        reportDialog.getUndo().startTransaction(UndoConstants.LAYER_DOWN);
        ...
        if (reportElements != null)
        {
            for (ReportElement reportElement : reportElements)
            {
                ...
                if (parent != null)
                {
                    parent.moveChildDown(reportElement);
                }
