...
public class MoveRightCommand extends AbstractCommand
{
    ...
    {
        super("MoveRightCommand");
        getTemplatePresentation().setText(TranslationManager.getInstance().getTranslation("R", "MoveRightCommand.Text"));
        getTemplatePresentation().setDescription(TranslationManager.getInstance().getTranslation("R", "MoveRightCommand.Description"));
        getTemplatePresentation().setMnemonic(TranslationManager.getInstance().getMnemonic("R", "MoveRightCommand.Text"));
        getTemplatePresentation().setDisplayedMnemonicIndex(TranslationManager.getInstance().getDisplayedMnemonicIndex("R", "MoveRightCommand.Text"));

        getTemplatePresentation().setAccelerator(KeyStroke.getKeyStroke(TranslationManager.getInstance().getTranslation("R", "MoveRightCommand.Accelerator")));
    }
    ...
    public void execute(@NotNull CommandEvent event)
    {
        ...
        if (reportElements != null)
        {
            for (ReportElement reportElement : reportElements)
            {
                double x = reportElement.getPosition().x + 10;
