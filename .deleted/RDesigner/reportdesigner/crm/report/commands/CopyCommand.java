...
public class CopyCommand extends AbstractCommand
{
    @NonNls
    @NotNull
    private static final Logger LOG = Logger.getLogger(CopyCommand.class.getName());


    public CopyCommand()
    {
        super("CopyCommand");
        getTemplatePresentation().setText(TranslationManager.getInstance().getTranslation("R", "CopyCommand.Text"));
        getTemplatePresentation().setDescription(TranslationManager.getInstance().getTranslation("R", "CopyCommand.Description"));
        getTemplatePresentation().setMnemonic(TranslationManager.getInstance().getMnemonic("R", "CopyCommand.Text"));
        getTemplatePresentation().setDisplayedMnemonicIndex(TranslationManager.getInstance().getDisplayedMnemonicIndex("R", "CopyCommand.Text"));

        getTemplatePresentation().setIcon(CommandSettings.SIZE_16, IconLoader.getInstance().getCopyIcon());

        getTemplatePresentation().setAccelerator(KeyStroke.getKeyStroke(TranslationManager.getInstance().getTranslation("R", "CopyCommand.Accelerator")));
    }
    ...
    public void update(@NotNull CommandEvent event)
    {
        ...
        if (reportElements != null && reportElements.length > 0)
        {
            ...
            {
                if (LOG.isLoggable(Level.FINE)) LOG.log(Level.FINE, "CopyCommand.update ", e);
