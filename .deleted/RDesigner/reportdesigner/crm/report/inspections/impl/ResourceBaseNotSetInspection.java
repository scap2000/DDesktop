 ...
 * Time: 19:24:51
 */
public class ResourceBaseNotSetInspection extends AbstractElementTestInspection
{
    ...
    protected void inspectElement(@NotNull ReportElement reportElement, @NotNull ArrayList<InspectionResult> inspectionResults)
    {
        ...
        {
            ResourceFieldReportElement resourceFieldReportElement = (ResourceFieldReportElement) reportElement;

            if ("".equals(resourceFieldReportElement.getResourceBase().trim()))
            {
                HashSet<LocationInfo> locationInfos = new HashSet<LocationInfo>();
                locationInfos.add(new LocationInfo(resourceFieldReportElement, PropertyKeys.RESOURCE_BASE));
                inspectionResults.add(new InspectionResult(InspectionResult.Severity.WARNING,
                                                           TranslationManager.getInstance().getTranslation("R", "ResourceBaseNotSetInspection.Summary"),
                                                           TranslationManager.getInstance().getTranslation("R", "ResourceBaseNotSetInspection.Description"),
                                                           null, locationInfos));
            }
        }
    }
