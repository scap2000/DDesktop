 ...
 * Time: 12:18:18
 */
public class FunctionNameTestInspection extends AbstractElementTestInspection
{
    protected void inspectElement(@NotNull ReportElement reportElement, @NotNull ArrayList<InspectionResult> inspectionResults)
    {
        if (reportElement instanceof ReportFunctionElement)
        {
            ...
            if (functionElement.getName().length() == 0)
            {
                ...
                locationInfos.add(new LocationInfo(functionElement, PropertyKeys.NAME));
                inspectionResults.add(new InspectionResult(InspectionResult.Severity.ERROR,
                                                           TranslationManager.getInstance().getTranslation("R", "FunctionNameTestInspection.Summary"),
                                                           TranslationManager.getInstance().getTranslation("R", "FunctionNameTestInspection.Description"),
                                                           null, locationInfos));
            }
        }
    }
