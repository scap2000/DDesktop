 ...
 * Time: 09:15:46
 */
public class LineDirectionTableCellRenderer extends DefaultTableCellRenderer
{
    @NotNull
    public Component getTableCellRendererComponent(@NotNull JTable table, @Nullable Object value, boolean isSelected, boolean hasFocus, int row, int column)
    {
        ...
        try
        {
            if (value instanceof HashSet)
            {
                ...
                if (hashSet.size() == 1)
                {
                    LineDirection text = (LineDirection) hashSet.iterator().next();
                    if (text != null)
                    {
                        label.setText(TranslationManager.getInstance().getTranslation("R", "LineDirection." + text.toString()));
                    }
                    else
                    {
                        label.setText("");
