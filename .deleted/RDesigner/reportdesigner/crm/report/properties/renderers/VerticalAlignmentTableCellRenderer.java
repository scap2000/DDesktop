 ...
 * Time: 09:15:46
 */
public class VerticalAlignmentTableCellRenderer extends DefaultTableCellRenderer
{
    @NotNull
    public Component getTableCellRendererComponent(@NotNull JTable table, @Nullable Object value, boolean isSelected, boolean hasFocus, int row, int column)
    {
        ...
        try
        {
            if (value instanceof HashSet)
            {
                ...
                if (hashSet.size() == 1)
                {
                    TextReportElementVerticalAlignment text = (TextReportElementVerticalAlignment) hashSet.iterator().next();
                    if (text != null)
                    {
