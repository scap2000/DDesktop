 ...
 * Time: 09:15:46
 */
public class Point2DTableCellRenderer extends DefaultTableCellRenderer
{
    @NotNull
    ...
    public Component getTableCellRendererComponent(@NotNull JTable table, @Nullable Object value, boolean isSelected, boolean hasFocus, int row, int column)
    {
        ...
        try
        {
            if (value instanceof HashSet)
            {
                ...
                if (hashSet.size() == 1)
                {
                    Point2D point = (Point2D) hashSet.iterator().next();
                    label.setText(decimalFormat.format(point.getX()) + "; " + decimalFormat.format(point.getY()));
                }
            }
        }
        catch (Throwable e)
