 ...
 * Time: 09:15:46
 */
public class Dimension2DTableCellRenderer extends DefaultTableCellRenderer
{
    @NotNull
    ...
    public Component getTableCellRendererComponent(@NotNull JTable table, @Nullable Object value, boolean isSelected, boolean hasFocus, int row, int column)
    {
        ...
        try
        {
            if (value instanceof HashSet)
            {
                ...
                if (hashSet.size() == 1)
                {
                    Dimension2D point = (Dimension2D) hashSet.iterator().next();
                    label.setText(decimalFormat.format(point.getWidth()) + "; " + decimalFormat.format(point.getHeight()));
                }
            }
        }
        catch (Throwable e)
