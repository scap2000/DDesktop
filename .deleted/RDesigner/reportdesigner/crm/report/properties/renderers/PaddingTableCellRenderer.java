    ...
    public Component getTableCellRendererComponent(@NotNull JTable table, @Nullable Object value, boolean isSelected, boolean hasFocus, int row, int column)
    {
        ...
        try
        {
            if (value instanceof HashSet)
            {
                ...
                if (hashSet.size() == 1)
                {
                    ElementPadding point = (ElementPadding) hashSet.iterator().next();
                    label.setText(decimalFormat.format(point.getTop()) + "; " + decimalFormat.format(point.getBottom()) + "; " + decimalFormat.format(point.getLeft()) + "; " + decimalFormat.format(point.getRight()));
                }
            }
        }
        catch (Throwable e)
