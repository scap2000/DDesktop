...
//pentru diverse functii utilitare
import java.lang.*;
...
/**********************************************************
 ...
 Informatii suplimentare: http://rowd.go.ro/proiecte
 **********************************************************/
public class orgChart extends JFrame {

    Chart chart = null;
    //obiectul Chart
    errorCanvas errCanv = null;
    //obiectul care afiseaza erorile
    ScrollPane scrollPane = null;
    //panou prevazut cu scrolling
    String nodes_s = null, error_msg = null, s = null;
    //obiecte string
    int nodes = 0, i = 0, j = 0;
    //obiecte integer
    StringTokenizer tk = null;
    //pentru impartirea parametrilor dupa '|'
    int maxDepth = 0;
    //adincimea maxima; necesara pentru calcularea dimeniunii panoului
    int cellWidth = 130;
    //definire zona de incadrare a unei celule, pentru desenare
    int cellHeight = 50;
    //important: doar calcularea coordonatelor tine seama de ea,
    //nu si desenarea propriu-zisa a celulelor
    String[][] ndata = null;
    //aici depozitam datele
    //array:                      param:
    //        0=id (1+)           X
    //        1=text              X
    //        2=parent id         X
    ...
    //        8=span              -
/*
  	pars[0] = "1|Monica|0|2|0|0";
	pars[1] = "2|Ion|1|0|3|1";
	pars[2] = "3|Mihai|1|6|4|1";
	pars[3] = "4|Filip|1|8|5|1";
	pars[4] = "5|Silvia|1|0|0|1";
	pars[5] = "6|Diana|3|0|7|2";
	pars[6] = "7|Simion|3|0|9|2";
	pars[7] = "8|Alexandru|4|0|0|2";
	pars[8] = "9|Mihaela|3|0|10|2";
	pars[9] = "10|Andrei|3|0|0|2";
 */
    public orgChart() {
	try {
	    jbInit();
	...
	} else if (nodes_s.equals(""))//daca nu contine nimic
	{
	...
	ndata = new String[nodes + 1][9];
	//defineste matricea de date
	String[] pars = new String[nodes];
	pars[0] = "1|Monica|0|2|0|0";
	pars[1] = "2|Ion|1|0|3|1";
	pars[2] = "3|Mihai|1|6|4|1";
	pars[3] = "4|Filip|1|8|5|1";
	...
	for (i = 1; i < nodes + 1; i++)//o parcurgem in sens crescator
	{
		...
		break;
	    } else if (s.equals(""))//daca nu contine nimic
	    {
		error_msg = "Need parameter '" + i + "' [5]";
		break;
	    }
	    tk = new StringTokenizer(s, "|", false);
	    //spargem informatia dupa |
	    ...
	    for (j = 6; j < 9; j++)
		ndata[i][j] = "1";
	    //completam restul cimpurilor cu 1 deocamdata
	}
	if (error_msg == null)//daca nu s-a dat nici un mesaj de eroare
	{
	    ...
	    //creare panou
	    add(scrollPane);
	    //introducere in desen
	    scrollPane.setBounds(0, 0, 610, 390);
	    //stabilim marimea de afisare
	    chart = new Chart(ndata, cellWidth, cellHeight, maxDepth);
	    //creare obiect Chart
	    scrollPane.add(chart);
	    //introducem Chart in panou
	} else {
	    errCanv = new errorCanvas(error_msg);
	    //cream obiectul pentru afisare erori
	    add(errCanv);
	    //il desenam
	}
    }
    ...
    private void makeSpan(int id) {
	int span = 0;
	//latimea pentru celula asta e 0 deocamdata
	int son_id = Integer.parseInt(ndata[id][3]);
	// cine e primul fiu
	while (son_id != 0)//atita vreme cit mai sint fii
	{
	    makeSpan(son_id);
	    // calculeaza latimea recursiv pentru fiu
	    span = span + Integer.parseInt(ndata[son_id][8]);
	    //creste latimea acestei celule cu latimea fiului
	    son_id = Integer.parseInt(ndata[son_id][4]);
	    // trece la urmatorul fiu
	}
	if (span == 0)
	    ndata[id][8] = "1";
	//daca n-a avut fii punem latime 1
	else
	    ndata[id][8] = span + "";
	//altfel punem latimea calculata
	if (Integer.parseInt(ndata[id][5]) > maxDepth)
	    maxDepth = Integer.parseInt(ndata[id][5]);
	//daca adincimea locala e mai mare decit cea mai mare inregistrata o retinem
    }

    private void setCoords(int id, int start_off) {
	int offset = start_off;
	//distanta fata de marginea din stinga
	int temp = 0;
	//variabila temporara
	int son_id = Integer.parseInt(ndata[id][3]);
	// cine e primul fiu
	...
	{
	    setCoords(son_id, offset);
	    // calculeaza pentru fiu si toti fiii lui
	    offset = offset + Integer.parseInt(ndata[son_id][8]) * cellWidth;
	    //recalculam distanta pina la margine
	    son_id = Integer.parseInt(ndata[son_id][4]);
	    // urmatorul fiu
	}
    }
...
class Chart extends Canvas {
    ...
    //matricea in care preluam datele
    private int maxX = 0;
    //latimea desenului
    private int maxY = 0;
    //inaltimea desenului
	...
	if (n_data == null)//daca nu primim nimic facem o matrice de baza
	{
	    ...
	    nodes = n_data;
	//altfel folosim ce primim
	maxX = cellWidth * Integer.parseInt(nodes[1][8]) + 30;
	//calculam latimea
	maxY = cellHeight * maxDepth + 45;
	//...si inaltimea desenului
	...
	//stabilim marimea desenului
    }

    private void drawText(int x, int y, String s, Graphics g) {
	Font font = new Font("Dialog", Font.PLAIN, 11);
	g.setFont(font);
    ...
    private void drawCell(int x, int y, String s, Graphics g) {
	g.setColor(new Color(255, 255, 255));
    ...
    private void chartDisplay(Graphics g) {
	int i = 0;
...
class errorCanvas extends Canvas {
    ...
    public errorCanvas(String em) {
	if (em == null)
	    ermsg = "Unknown error [?1]";
	else if (em.equals(""))
	    ermsg = "Unknown error [?2]";
	else
	    ermsg = em;
    }
