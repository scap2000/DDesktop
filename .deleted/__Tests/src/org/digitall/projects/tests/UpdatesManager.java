...
public class UpdatesManager extends JDialog implements Observer {
    ...
    private JButton pauseButton, resumeButton, updateButton;
	...
	if (selectedDownload != null)
	    selectedDownload.deleteObserver(UpdatesManager.this);
	/* If not in the middle of clearing a download,
       set the selected download and register to
       receive notifications from it. */
	if (!clearing && table.getSelectedRow() > -1) {
	    selectedDownload = tableModel.getDownload(table.getSelectedRow());
	    selectedDownload.addObserver(UpdatesManager.this);
	    updateButtons();
	}
    }
	...
	UpdatesManager manager = new UpdatesManager();
	...
	try {
	    File file = new File("DDFileList.xml");
	    DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
	    DocumentBuilder db = dbf.newDocumentBuilder();
	    Document doc = db.parse(file);
	    doc.getDocumentElement().normalize();
	    System.out.println("Root element " + doc.getDocumentElement().getNodeName());
	    NodeList nodeLst = doc.getElementsByTagName("filelist");
	    System.out.println("Information of all files");
	    for (int s = 0; s < nodeLst.getLength(); s++) {
		Node fstNode = nodeLst.item(s);
		if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
		    Element fstElmnt = (Element)fstNode;
		    NodeList fstNmElmntLst = fstElmnt.getElementsByTagName("filename");
		    Element fstNmElmnt = (Element)fstNmElmntLst.item(0);
		    NodeList fstNm = fstNmElmnt.getChildNodes();
		    System.out.println("FileName: " + ((Node)fstNm.item(0)).getNodeValue());
		    NodeList lstNmElmntLst = fstElmnt.getElementsByTagName("path");
		    Element lstNmElmnt = (Element)lstNmElmntLst.item(0);
		    NodeList lstNm = lstNmElmnt.getChildNodes();
		    System.out.println("Path: " + ((Node)lstNm.item(0)).getNodeValue());
		    NodeList lstNmElmntLst2 = fstElmnt.getElementsByTagName("hash");
		    Element lstNmElmnt2 = (Element)lstNmElmntLst2.item(0);
		    NodeList lstNm2 = lstNmElmnt2.getChildNodes();
		    System.out.println("Hash: " + ((Node)lstNm2.item(0)).getNodeValue());
		}
	    }
	} catch (Exception e) {
	    e.printStackTrace();
    ...
    public void retrieveXMLFileList() {
	//Download xmlFileList = new Download(verifyUrl("http://digitallsh.com.ar/digitall/main/ddesktop/deploy/DDFileList.xml"));
	Download xmlFileList = new Download(verifyUrl("http://ftp.gnu.org/gnu/automake/automake-1.10.tar.bz2"));
	tableModel.addDownload(xmlFileList);
	while (xmlFileList.getStatus() != Download.COMPLETE) {
	    try {
