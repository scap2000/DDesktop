...
public class DrawDesk extends BasicContainerPanel {
    ...
    private BasicButton bimportar = new BasicButton();
    private BasicButton bexportar = new BasicButton();
    ...
    public DrawDesk(BasicLabel _statusBar, int _idProject) {
	try {
	...
	Boton1.setText("_");
	Boton1.setBounds(new Rectangle(5, 570, 88, 25));
	...
	labelinfo.setBounds(new Rectangle(15, 40, 760, 20));
	bparalela.setText("paralela");
	bparalela.setBounds(new Rectangle(695, 195, 88, 25));
	...
	bNumera.setBounds(new Rectangle(680, 385, 105, 20));
	...
	btrapeciorect.setBounds(new Rectangle(695, 230, 88, 25));
	...
	bimportar.setText("SHP2PGSQL");
	bimportar.setBounds(new Rectangle(680, 285, 105, 25));
	bexportar.setText("PGSQL2SHP");
	bexportar.setBounds(new Rectangle(680, 320, 105, 25));
	bnuevo.setText("nuevo");
	bnuevo.setBounds(new Rectangle(695, 160, 88, 25));
	...
	bparcela.setBounds(new Rectangle(695, 125, 88, 25));
	...
	belimina.setBounds(new Rectangle(695, 95, 90, 25));
	...
	bmalla.setBounds(new Rectangle(695, 35, 88, 25));
	...
	this.add(bNumera, null);
	this.add(bInfo, null);
	this.add(bexportar, null);
	this.add(bimportar, null);
	this.add(btrapeciorect, null);
	this.add(bparalela, null);
	this.add(bnuevo, null);
	this.add(bparcela, null);
	this.add(belimina, null);
	this.add(bpoligono, null);
	this.add(bmalla, null);
	add(labelinfo, null);
	add(labelxy, null);
	this.add(Boton1, null);
	...
	if (currentPosition != null) {
		...
		/*                g2.setColor(Color.black);
                g2
                .drawString("(" + decimalFormat(pos.getX(), 4) + " " +
                              decimalFormat(pos.getY(), 4) + ")::(" + latlong + ")",
    ...
    */
    ...
    public void doPolys() {
	try {
	    String Data = "the_geom from (select the_geom from mapper.project_geoms " + " where idproject = " + idProject + ") as foo";
	    Data = Data.trim();
	    ...
	    if (Poligonos.size() == 0) {
		Advisor.messageBox("No hay polÃ­gonos para ver", "Tabla vacÃ­a");
	    }
	...
	for (int i = 0; i < _poly.getVertexCount() * 2; i += 2) {
	    ...
	    /*drawScale **/xy[i + 1] = (fHeight - ((_poly.getY(i / 2) - yOffset) * drawFactor));
	}
	...
	if (cantidad > 0) {
		...
		/*drawScale **/xy[i + 1] = (fHeight - ((yp[i / 2] - yOffset) * drawFactor));
	    }
	    Polygon2D poly = new Polygon2D.Double(xy);
	    return poly;
	} else {
	...
	int y = /*drawScale * */(int)((fHeight - ((yd - yOffset) * drawFactor)));
	...
	int y = /*drawScale * */(int)((fHeight - ((xy.getY() - yOffset) * drawFactor)));
	...
	return /*drawScale * */(int)((fHeight - ((y - yOffset) * drawFactor)));
    }
	...
	double area = 0;
	double angulo = 0;
	if (distancePoints.size() < 99)//Fijarse si hay que mantener la restricciÃ³n o no
	{
	    if (nuevo) {
	    ...
	    labelinfo.setText("Distancia parcial: " + decimalFormat(distancia, 4) + "m - Distancia total: " + decimalFormat(distanciaTotal, 4) + "m - Area: " + decimalFormat(area, 4) + "m² - Angulo: " + decimalFormat(angulo, 4));
	} else {
    ...
    /**  private Point2D[] calcgrid() 
  {
    ...
    protected MouseListener MallaListener = new MouseListener() {
		...
		if (me.getButton() == me.BUTTON3) {
		    ...
		    parcelasTemporales.removeAllElements();
		    //Vacio los datos de la operaciÃ³n actual
    ...
    protected MouseMotionListener MallaMotionListener = new MouseMotionListener() {
		...
		labelinfo.setText("Area mallada: " + String.valueOf(decimalFormat(areamallada, 4)) + " m² Area restante: " + String.valueOf(decimalFormat(arearestante, 4)));
