...
public class DrawPanel extends BasicContainerPanel {

    private JToolBar tbStatus = new JToolBar();
    private BorderLayout borderLayout1 = new BorderLayout();
    private JDesktopPane desktop;
    private String projectName;
    private Vector loadedDrills = new Vector();
    private Vector loadedDrillsID = new Vector();
    private CoordinateViewer coordinateViewer;

    public DrawPanel(JDesktopPane _desktop, BasicLabel _statusBar, int _idProject, String _projectName) {
	try {
	    desktop = _desktop;
	...
	for (int puntos = 0; puntos < Puntos.size(); puntos++) {
	    DrillPoint2D p = (DrillPoint2D)Puntos.elementAt(puntos);
	    Shape drill = new Ellipse2D.Double(xtoPixel(p.getX()) - drillSize / 2, ytoPixel(p.getY()) - drillSize / 2, drillSize, drillSize);
	    if (containedPoint != -1) {
		g2.setColor(Color.magenta);
		...
		if (Integer.parseInt(loadedDrillsID.elementAt(i).toString()) == p.getIdPoint()) {
		    if (((DrillData)loadedDrills.elementAt(i)).isVisible()) {
	...
	if (currentPosition != null) {
	    /*LatLongCoord latlon = geo.utm2geo(currentPosition.getX(), currentPosition.getY(), zona);
            String latlong = geo.dec2gms(latlon.getLatitude(), 4) + " lat " + geo.dec2gms(latlon.getLongitude(), 4) + " long";
            g2.setColor(Color.black);
	    ...
	    coordinateViewer.setTitle(projectName);
	}
	//Dibujo el texto del puntero del mouse
    ...
    */
    ...
    public void doPolys() {
	try {
	    String Data = "the_geom from (select utm as the_geom from drilling.drills " + " where idproject = " + idProject + ") as foo";
	    Data = Data.trim();
	...
	try {
	    String Data = " the_geom from (select utm as the_geom, iddrill, ddh from drilling.drills " + " where idproject = " + idProject + ") as foo";
	    Data = Data.trim();
	    String geometry = " the_geom ";
	    //Data.substring(0, Data.indexOf(" ")).trim();
	    Data = Data.substring(Data.indexOf(" ")).trim();
	    String LabelItem = "iddrill, ddh, ";
	    String Qwhere = "";
	    String Query = "select " + LabelItem + geometry + ", X(" + geometry + "), Y(" + geometry + ") " + Data + Qwhere;
	    System.out.println(Query);
	    ResultSet count = org.digitall.lib.sql.LibSQL.exQuery("select count(*) from (" + Query + ") as foo");
	    count.next();
	    ResultSet Points = org.digitall.lib.sql.LibSQL.exQuery(Query);
	    if (Qwhere == Qwhere) {
		Query = "select " + "xmin(extent(" + geometry + ")), " + "xmax(extent(" + geometry + ")), " + "ymin(extent(" + geometry + ")), " + "ymax(extent(" + geometry + ")) " + Data;
		System.out.println(Query);
		ResultSet Extent = org.digitall.lib.sql.LibSQL.exQuery(Query);
		Extent.next();
		xMin = Extent.getDouble("xmin");
		xMax = Extent.getDouble("xmax");
		yMin = Extent.getDouble("ymin");
	    ...
	    while (Points.next()) {
		/**Punto*/
		DrillPoint2D punto = new DrillPoint2D.Double(Points.getDouble("X"), Points.getDouble("Y"));
		addOsnapPoint(new Point2D.Double(punto.getX(), punto.getY()));
		punto.setIdPoint(Points.getInt("iddrill"));
		punto.setName(Points.getString("ddh"));
    ...
    /** private File selectFile()
  {
    ...
    protected MouseListener InfoListener = new MouseListener() {
		...
		if (containedPoint != -1) {
			...
			//Show drill information!
			ShowData showData = new ShowData(idProject, ((DrillPoint2D)Puntos.elementAt(containedPoint)).getIdPoint(), ((DrillPoint2D)Puntos.elementAt(containedPoint)).getName());
			showData.setModal(true);
			showData.setVisible(true);
		    } else if (me.getButton() == me.BUTTON1 && me.getClickCount() == 1) {
			int idDrill = ((DrillPoint2D)Puntos.elementAt(containedPoint)).getIdPoint();
			int i = 0;
			boolean found = false;
			while (i < loadedDrillsID.size() && !found) {
			    if (Integer.parseInt(loadedDrillsID.elementAt(i).toString()) == idDrill) {
				found = true;
				DrillData _temp = ((DrillData)loadedDrills.elementAt(i));
				if (!_temp.isVisible()) {
			...
			if (!found) {
			    DrillData _temp = new DrillData(projectName);
			    _temp.setIdDrill(idDrill);
			    desktop.add(_temp);
			    _temp.setVisible(true);
			    loadedDrills.addElement(_temp);
			    loadedDrillsID.addElement(String.valueOf(idDrill));
			}
		    }
		}
	    }
    ...
    protected MouseMotionListener InfoMotionListener = new MouseMotionListener() {
		...
		for (int i = 0; i < Puntos.size(); i++) {
		    DrillPoint2D p = (DrillPoint2D)Puntos.elementAt(i);
		    Shape drill = new Ellipse2D.Double(xtoPixel(p.getX()) - drillSize / 2, ytoPixel(p.getY()) - drillSize / 2, drillSize, drillSize);
		    if (drill.contains(me.getPoint())) {
			containedPoint = i;
		    }
		    /*if (((Point2D)Puntos.elementAt(i)).distance(toSpace(me.getPoint())) <= osnapRectSize) {
                        containedPoint = i;
                    }*/
		}
		repaint();
    ...
    public void setDesktop(JDesktopPane _desktop) {
	desktop = _desktop;
    }
