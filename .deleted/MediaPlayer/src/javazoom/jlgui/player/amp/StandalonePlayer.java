...
public class StandalonePlayer extends BasicInternalFrame implements Loader {
    ...
    // 1, 2, for different Volume.bmp
    private boolean autoRun = false;
    /*-- Front-end --*/
    private PlayerUI mp = null;
    private BasicInternalFrame eqWin = new BasicInternalFrame();
    private BasicInternalFrame plWin = new BasicInternalFrame();
	...
	if ((initSong != null) && (!initSong.equals("")))
	    playlistfound = mp.loadPlaylist(initSong);
	else
	    playlistfound = mp.loadPlaylist(config.getPlaylistFilename());
    }
	...
	if (eqWin != null) {
	    eqWin.setVisible(config.isEqualizerEnabled() && _visible);
	}
	if (plWin != null) {
	    plWin.setVisible(config.isPlaylistEnabled() && _visible);
	}
