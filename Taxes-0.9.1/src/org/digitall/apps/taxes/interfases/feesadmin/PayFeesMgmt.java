...
public class PayFeesMgmt extends BasicPrimitivePanel {
    ...
    private AcceptButton btnLibreDeuda = new AcceptButton();
	...
	btnNext.addActionListener(new ActionListener() {
        ...
        tfAmount.setBounds(new Rectangle(320, 5, 130, 35));
        tfAccruedInterest.setBounds(new Rectangle(463, 5, 130, 35));
        tfTotalAmount.setBounds(new Rectangle(760, 5, 130, 35));
        tfAccruedDiscount.setBounds(new Rectangle(605, 5, 130, 35));
	...
	addButton(btnLibreDeuda);
	btnLibreDeuda.setVisible(true);
	btnLibreDeuda.setToolTipText("Imprimir Libre Deuda");
    ...
    private void calcTotalAmount(){// calcTotalAmount
	 if (cbCantAnticipos.getSelectedIndex() >= 0) {
	     ...
	     totalDescuento = Format.toDouble(totalValor.add(totalIntereses).doubleValue() * coordinador.getPorcentajeDto());
	...
	((TaxesMain)getTabContainer()).getPayAnnualFeesMgmt().setCoordinador(coordinador);
	((TaxesMain)getTabContainer()).setEnabledPanels(5);
    }

    private void btnLibreDeuda_actionPerformed(ActionEvent e) {
	switch (coordinador.getIdTipoImpuesto())  {
	    case 1: generarLibreDeuda(3,1);   
	    break;
	    case 2: generarLibreDeuda(4,2);   
	    break;
	    case 3: generarLibreDeuda(1,3);   
	    break;
	}
    }
	...
	if (boletaContribucion.saveData() > 0)  {	
	   ...
	   Advisor.messageBox("Se generó correctamente el libre deuda con la boleta Nº "+ boletaContribucion.getNumeroFormateado() ,"Aviso");
	} else  {
	...
	if ( cantCuotas > 0)  {
		if(Advisor.question("Aviso","¿Está seguro de generar la boleta para "+ cantCuotas +" anticipos por un total de $"+ tfTotalAmount.getValue()+"?")){
	...
	if ((coordinador.getMoratoria().getIdTipoPlanDePago() > 0) && (cantCuotas > 0))  {
    ...
    private void verificarDeuda(){
        btnLibreDeuda.setEnabled(LibSQL.getBoolean("taxes.getLibreDeuda",""+ coordinador.getIdTipoImpuesto() +","+ coordinador.getIdBien()));
    }

    private void verificarPagoAnual() {
	btnNext.setEnabled(LibSQL.getBoolean("taxes.getEstadoPagoAnual",""+ coordinador.getIdTipoImpuesto() +","+ coordinador.getIdBien()));
    }
	...
	boletaAutomotor.setTotal(new BigDecimal("" + Double.parseDouble(tfTotalAmount.getValue().toString())).doubleValue());//cambiar
