...
public class PaymentOrderManualMgmt extends BasicPrimitivePanel {
    ...
    private int[] sizeColumnList = { 211, 83 , 107 , 140 , 97 };
    private Vector dataRowDetail = new Vector();
    private GridPanel listDetailPanel = new GridPanel(5000, sizeColumnList, "", dataRowDetail);
    private Vector<String> headerListDetail = new Vector<String>();
    
    private int [] sizeColumnListConcept = { 211, 83, 107, 140, 97};
    ...
    private Vector<String> headerListDetailConcept = new Vector<String>();
    ...
    private Vector<String> headerListDeduction = new Vector<String>();
    ...
    private Vector<String> headerListPaymentWay = new Vector<String>();
    ...
    private Vector<String> headerListBookKeepingEntry = new Vector<String>();
	...
	btnAddDetail.setToolTipText("Agregar monto al Debe");
	...
	/*listDetailPanel.getTable().addMouseListener(new MouseAdapter() {
        ...
        listDetailPanel.getTable().addMouseMotionListener(new MouseMotionAdapter() {
    ...
    private void agregarSubTotalFacturas(double amount) {
        if (rbtnFacturas.isSelected()) { //FACTURAS COMPLETAS
            tfDetailPartialAmount.setValue(tfDetailPartialAmount.getAmount() + amount);
            tfSubtotal.setValue(0.0);
            tfSubtotal.setBackground(Color.LIGHT_GRAY);
        } else { //FACTURAS PARCIALES
            tfSubtotal.setValue(tfSubtotal.getAmount() + amount);
            if (tfSubtotal.getAmount() > Double.parseDouble(tfDetailPartialAmount.getValue().toString())) {
	...
	Double subTotal = getSelectedAmount();
	if (rbtnConcepto.isSelected()) {
    ...
    private boolean agregarEnAsiento(int idFactura, double amount){
        paymentOrderDetail = new PaymentOrderDetail();
        //paymentOrderDetail.setAccount(new Account(Integer.parseInt(cbAccount.getSelectedValue().toString())));
        paymentOrderDetail.setAccount(new Account(Integer.parseInt(cbProvider.getSelectedValueRef().toString().split("#")[1])));
        paymentOrderDetail.setVoucher(new Voucher(idFactura));
        idBudget = -1;
        paymentOrderDetail.setPaymentOrder(paymentOrder);
        paymentOrderDetail.setAmount(amount);
    ...
    */
	...
	if (!cbProvider.getSelectedValue().equals("-1")) {
		...
		paymentOrderDetail = null;
                ...
                if (rbtnFacturas.isSelected()) {
                        //AGREGAR 
                        Vector selecteds = listDetailPanel.getSelectedsVector();
                        for (int i = 0; i < selecteds.size(); i++) {
                            Vector selected = (Vector)selecteds.elementAt(i);
                            int idFactura = Integer.parseInt(selected.elementAt(0).toString());
                            double amount = Double.parseDouble(selected.elementAt(5).toString());
                            if (agregarEnAsiento(idFactura, amount)) {
                                agregarSubTotalFacturas(amount);
                            };
                        }
                        updateDetailAmount();
                        loadComboAccountsCash();
                ...
                } else if (rbtnConcepto.isSelected()) {
	...
	if (paymentOrder == null) {
	    ...
	    tabbedPane.setSelectedIndex(0);
            ...
            if (tfDetailAmount.getAmount() <= 0) {
                Advisor.messageBox("El campo ($) Monto debe ser mayor que cero", "Error");
            } else if (tfConcept.getString().trim().equals("")) {
                Advisor.messageBox("Debe completar los campos\n Observaciones, Concepto", "Error");
            } else if (tfDetailAmount.getAmount() > selectedAmount) {
                Advisor.messageBox("El campo ($) Monto no puede superar el valor de las facturas seleccionadas: " + Format.money(selectedAmount), "Error");
            } else {
                if(saveDetail()){
                     if(rbtnMonto.isSelected()) {
	...
	if (paymentOrder != null) {
		...
		if (rbtnMonto.isSelected())  {
			...
			if (tfAvailableAmount.getAmount() == 0) { //(Format.toDouble(tfSubtotal.getAmount()).doubleValue()) >= (Format.toDouble(tfDetailAmount.getAmount()).doubleValue()))  {
	...
	if (rbtnConcepto.isSelected()) {
        ...
        listDetailPanel.setEnabled(_valor);
        for (int i = tabbedPane.getTabCount(); i > 1; i--) {
            tabbedPane.setEnabledAt(i-1, !_valor);
        }
	...
	if (cbProvider.getSelectedIndex() != -1) {
            ...
            rbtnFacturas.setText("O.P. x Facturas Completas");
            rbtnMonto.setText("O.P. x Facturas Parciales");
	    if ( (!rbtnConcepto.isSelected() && (listDetailPanel.getSelectedsID().size() > 0)) || (rbtnConcepto.isSelected()) ) { //(!tfObservations.getString().toString().equals("") ) {
		if (paymentOrder == null) {
		    if (saveHeaderData()) {
			enabledHeader(false);
			if (rbtnFacturas.isSelected()) {
		...
		Advisor.messageBox("Para Órdenes por facturas (completas o parciales) debe primero seleccionar una o más facturas","Aviso");
	    }
	} else {
